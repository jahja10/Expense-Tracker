...
import { Check, CreditCard, List, Pencil, Plus, Trash2, X } from "lucide-react"
...
export default function PaymentMethods() {
  ...
  useEffect(() => {
    const loadPaymentMethods = async () => {
      try {
        ...
        const result: PaymentMethod[] = data.paymentMethods ?? []
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/PaymentMethods/${deleteId}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      setPaymentMethods((prev) =>
        prev.filter((paymentMethod) => paymentMethod.id !== deleteId)
      )
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = isEditing
        ? `${apiUrl}/PaymentMethods/${editingId}`
        : `${apiUrl}/PaymentMethods`

      const method = isEditing ? "PUT" : "POST"

      const body: CreatePaymentMethodRequest | UpdatePaymentMethodRequest = {
        name: name.trim(),
      }
      ...
      const savedPaymentMethod: PaymentMethod = await response.json()

      if (isEditing) {
        setPaymentMethods((prev) =>
          prev.map((paymentMethod) =>
            paymentMethod.id === editingId
              ? savedPaymentMethod
              : paymentMethod
          )
        )
      } else {
        setPaymentMethods((prev) => [savedPaymentMethod, ...prev])
      }
          ...
          <div className="relative overflow-hidden rounded-[28px] border border-white/10 bg-white/5 px-6 py-6 shadow-[0_15px_40px_rgba(0,0,0,0.35)] backdrop-blur-2xl">
            ...
            <div className="relative z-10">
              ...
              {loadingPaymentMethods ? (
                ...
                <p className="text-sm text-white/60">No payment methods yet.</p>
              ) : (
                <div className="h-[390px] overflow-y-scroll pr-2 [scrollbar-width:thin] [scrollbar-color:rgba(52,211,153,0.35)_transparent] [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-white/10 hover:[&::-webkit-scrollbar-thumb]:bg-emerald-400/30">
                  <div className="flex flex-col gap-2.5">
                    {paymentMethods.map((paymentMethod) => {
                      const isEditingThis = editingId === paymentMethod.id
          ...
          <div className="relative z-10 w-full max-w-md overflow-hidden rounded-[28px] border border-white/10 bg-white/5 p-6 shadow-[0_20px_60px_rgba(0,0,0,0.6)] backdrop-blur-2xl">
            ...
            <div className="relative z-10">
              ...
              <p className="text-sm leading-6 text-white/60">
                This action cannot be undone. The selected payment method will
